...
kc_include_once(KANCART_ROOT . '/modules/paypal/paypal_lib.php');
kc_include_once(KANCART_ROOT . '/modules/paypal/paypal.php');

class PaypalExpressCheckout extends KPayPal {
    ...
    public function __construct($returnUrl = null, $cancelUrl = null, $type = FALSE) {
        parent::__construct();

        if ($returnUrl === null || $cancelUrl === null) {
            $this->returnUrl = $this->context->cookie->returnUrl;
            $this->cancelUrl = $this->context->cookie->cancelUrl;
        } else {
            $this->context->cookie->returnUrl = $this->returnUrl = $returnUrl;
            $this->context->cookie->cancelUrl = $this->cancelUrl = $cancelUrl;
        }
    ...
    public function setExpressCheckout() {
        ...
        $fields['CANCELURL'] = $this->cancelUrl;
    ...
    public function getExpressCheckout() {
        ...
        if ($this->hasSucceedRequest()) {
            $this->ready = TRUE;
            $this->payer_id = $this->result['PAYERID'];
        }
    ...
    private function callAPI($fields) {
        $this->logs = array();
        $paypal_lib = new KPaypalLib($this);
    ...
    private function _setPaymentDetails(&$fields) {
        // Required field
        $fields['RETURNURL'] = $this->returnUrl;
    ...
    public function getRedirectUrl() {
        ...
        $url = '/cgi-bin/webscr?cmd=_express-checkout-mobile&useraction=continue';
        return 'https://' . $this->getPayPalURL() . $url . '&token=' . urldecode($this->token);
    }
